...
jobs:
  publish:
    ...
    steps:
      ...
      - name: Publish
        run: uvx --from release-devkit==0.1.10 publish-stable --config publish-config.json
```
...
```bash
uvx --from release-devkit==0.1.10 publish-stable --config build/publish-config.json
```
...
```bash
uvx --from release-devkit==0.1.10 publish-stable --config build/publish-config.json
uvx --from release-devkit==0.1.10 publish-dev --config build/publish-config.json --run-id ${{ github.event.workflow_run.id }}
uvx --from release-devkit==0.1.10 create-release --config build/publish-config.json
```
